...
nodes_file=fopen("C:\\Users\\zhang\\Desktop\\easymesh\\exampleedge.n","r");
...
int num_sides;
sides_file=fopen("C:\\Users\\zhang\\Desktop\\easymesh\\exampleedge.s","r");
if(NULL==sides_file)                                                                       
  {                                                                             
...
double interiorsides[num_sides];
...
elements_file=fopen("C:\\Users\\zhang\\Desktop\\easymesh\\exampleedge.e","r");
...
for(i=0;i<interior;i++)
{printf("%d",interiorsides[i]);system("pause");
}
/* 
system("pause");
...
for(i=0;i<interior;i++)
{printf("%d",sides[interiorsides[i]].x);system("pause");
}
...
double kk=1.0/8.0;int s,r,ceshi=-1;double err1;
...
for(r=0;r<3;r++)
{
 ...
 emm[0][0]=localength[0]*localength[0]*(fff[1][1]-fff[0][1]+fff[0][0])*(6*delta);

 emm[0][1]=sign(eptr[0])*sign(eptr[1])*localength[0]*localength[1]*(fff[1][2]-fff[1][1]-2*fff[0][2]+fff[0][1])*(12*delta);
 
 emm[0][2]=sign(eptr[0])*sign(eptr[2])*localength[0]*localength[2]*(fff[1][0]-2*fff[1][2]-fff[0][0]+fff[0][2])*(12*delta); 

 
 emm[1][1]=localength[1]*localength[1]*(fff[2][2]-fff[1][2]+fff[1][1])*(6*delta);
   
 emm[1][2]=sign(eptr[2])*sign(eptr[1])*localength[2]*localength[1]*(fff[2][0]-fff[1][1]-2*fff[1][0]+fff[1][2])*(12*delta);

 emm[2][2]=localength[2]*localength[2]*(fff[0][0]-fff[0][2]+fff[2][2])*(6*delta);
...
{ if(localength[s]>=0)
	{j=sides[abs(eptr[s])-1].ptr-1;
	   umid[0] += delta*sign(eptr[s])*localength[s]*  U[j]*(aaa[s]*invM[0][(1+s)%3]-aaa[(1+s)%3]*invM[0][s]+y0*(invM[1][s]*invM[0][(1+s)%3]-invM[1][(1+s)%3]*invM[0][s]));
	  
	  
	  umid[1] += delta*sign(eptr[s])*localength[s]* U[j]* (aaa[s]*invM[1][(1+s)%3]-aaa[(1+s)%3]*invM[1][s]+x0*(invM[1][(1+s)%3]*invM[0][s]-invM[1][s]*invM[0][(1+s)%3]))  ;
//printf("%f,\n",U[j]);
  ...
  err1+=abs(1-y0*y0-umid[0])+abs(1-x0*x0-umid[1]);
 ...
 if (!outL)
    {
	  ...
	  for (i = 0; i <interior; i++)
	  {for (j = 0; j <num_free; j++)
	  fprintf(outC, "%12.12f ",C[i][j]);fputc('\n',outC);}
