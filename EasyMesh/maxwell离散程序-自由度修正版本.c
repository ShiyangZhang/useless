...
typedef struct{  
    Triple data[MAXSIZE];//三元组表，data[0]可用  
...
int arg;
 if(argc!=4)
  {printf("程序名称之后，输入以 .n .s .e为后缀的文件的文件名，并用空格隔开");
...
TSMatrix B=NewMatrix(num_free,interior);
TSMatrix C=NewMatrix(interior,num_free);
TSMatrix Pcurl=NewMatrix(interior,2*num_free);

TSMatrix  L=NewMatrix(num_free,num_free);
TSMatrix  ML=NewMatrix(num_free,num_free);
