...
EasyMesh使用方法：编译EasyMesh后 在EasyMesh.exe所在文件夹按Shift+右键，打开  在此处打开命令窗口（W） 选项  输入 EasyMesh L4 +dxf  (此处L4代表L4.d,为EasyMesh输入文件)
C:\\Users\\zhang\\Desktop\\easymesh\\L4.n  
C:\\Users\\zhang\\Desktop\\easymesh\\L4.s  
C:\\Users\\zhang\\Desktop\\easymesh\\L4.e  
请在下面程序中搜索L4，更改您的相应文件名和文件位置
并且更改参数kk的值。（论文中k值），
...
nodes_file=fopen("C:\\Users\\zhang\\Desktop\\easymesh\\L4.n","r");
...
int num_sides;
sides_file=fopen("C:\\Users\\zhang\\Desktop\\easymesh\\L4.s","r");
if(NULL==sides_file)                                                                       
  {                                                                             
...
for(i=0;i<num_sides;i++)
{
	  load_s(sides_file, dummy1);     
	   ...
	   if(1){fprintf(outXs, "%f  %f %f %f  ", nodes[sides[i].x].x,nodes[sides[i].x].y,nodes[sides[i].y].x,nodes[sides[i].y].y);  fputc('\n',outXs); // printf("dsfdsadfasdfasdf %d",dummymark);system("pause");
...
elements_file=fopen("C:\\Users\\zhang\\Desktop\\easymesh\\L4.e","r");
...
double* * ML=NULL;
   ML = (double **)malloc(sizeof(double *)*num_free);
	for (i=0; i<num_free; i++)
	{ML[i] = (double *)malloc(sizeof(double)*num_free); memset(ML[i],0,num_free*sizeof(double)); } 
...
if(eptr[0]>0)
	     {indices[0]=sides[eptr[0]-1].x;indices[1]=sides[eptr[0]-1].y;  if(1) localength[0]=length[sides[eptr[0]-1].ptr-1]; else localength[0]=-1;}
 else {indices[0]=sides[-eptr[0]-1].y;indices[1]=sides[-eptr[0]-1].x;if(1) localength[0]=length[sides[-eptr[0]-1].ptr-1]; else localength[0]=-1;}
		   
if(eptr[1]>0)
		 {indices[2]=sides[eptr[1]-1].y;if(1) localength[1]=length[sides[eptr[1]-1].ptr-1]; else localength[1]=-1;}
else {indices[2]=sides[-eptr[1]-1].x;if(1) localength[1]=length[sides[-eptr[1]-1].ptr-1]; else localength[1]=-1;}
	 
if(eptr[2]>0) {if(1) localength[2]=length[sides[eptr[2]-1].ptr-1]; else localength[2]=-1;}
else {if(1) localength[2]=length[sides[-eptr[2]-1].ptr-1]; else localength[2]=-1;}
...
FILE *out;
 ...
 if (!out)
	...
	//printf("%d",interior);system("pause");
	FILE *outA;
 ...
 if (!outML)
    {
	  ...
	  for (i = 0; i <num_free; i++)
	  {for (j = 0; j <num_free; j++)
	  fprintf(outML, "%20.12f  ",ML[i][j]);fputc('\n',outML);}
